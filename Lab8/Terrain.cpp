#include "Terrain.h"

// Конструкторы класса Terrain
Terrain::Terrain(int widthIn, int heightIn, int stepSizeIn)
{
    width = widthIn;
    height = heightIn;
    stepSize = stepSizeIn;
    makeVertices(&vertices);
}

Terrain::Terrain() {
    width = 50;
    height = 50;
    stepSize = 10;
    makeVertices(&vertices);
}

// Метод getVAO создает и возвращает VAO (Vertex Array Object) для отрисовки ландшафта
unsigned int Terrain::getVAO() {
    // Создаем VAO и VBO
    glGenVertexArrays(1, &VAO);
    glGenBuffers(1, &VBO);
    // Привязываем VAO и VBO
    glBindVertexArray(VAO);
    glBindBuffer(GL_ARRAY_BUFFER, VBO);
    // Заполняем буфер данными о вершинах
    glBufferData(GL_ARRAY_BUFFER, (vertices.size() * sizeof(GLfloat)), vertices.data(), GL_STATIC_DRAW);

    // Устанавливаем указатели на атрибуты вершин в буфере
    // Первый атрибут - координаты xyz
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)0);
    glEnableVertexAttribArray(0);
    // Второй атрибут - текстурные координаты uv
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)(3 * sizeof(float)));
    glEnableVertexAttribArray(1);

    // Отвязываем VAO и VBO
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    glBindVertexArray(0);

    return VAO;
}

// Метод getSize возвращает количество вершин в векторе vertices
int Terrain::getSize() {
    return vertices.size();
}

// Метод getVertices возвращает вектор вершин
std::vector<float> Terrain::getVertices() {
    return vertices;
}

// Метод makeVertices заполняет вектор вершин
void Terrain::makeVertices(std::vector<float>* vertices) {
    /* triangle a b c
         b
         | \
         a _ c


       triangle d f e
         f _ e
         \ |
           d

       c == d
       b == f
       Duplicate vertices but easier in long run! (tesselation and LOD)

      a = (x,y,z)
      b = (x, y+1)
      c = (x+1,y)

      d = (x+1,y)
      e = (x, y+1)
      f = (x+1,y+1)

       each vertex a, b,c, etc. will have 5 data:
       x y z u v
        */

        // Заполняем вектор вершин
    for (int y = 0; y < height - 1; y++) {
        float  offSetY = y * stepSize;
        for (int x = 0; x < width - 1; x++) {
            float offSetX = x * stepSize;
            makeVertex(offSetX, offSetY, vertices);  // a
            makeVertex(offSetX, offSetY + stepSize, vertices);  // b
            makeVertex(offSetX + stepSize, offSetY, vertices);   // c
            makeVertex(offSetX + stepSize, offSetY, vertices);  //d
            makeVertex(offSetX, offSetY + stepSize, vertices);  //e
            makeVertex(offSetX + stepSize, offSetY + stepSize, vertices);  //f
        }
    }
}

// Метод makeVertex добавляет вершину с заданными координатами и текстурными координатами в вектор vertices
void Terrain::makeVertex(int x, int y, std::vector<float>* vertices) {
    // Вычисляем шум Перлина для заданных координат
    double pn = perlin.noise(x, y, 0.5);

    // Добавляем координаты xyz в вектор вершин
    vertices->push_back((float)x); //xPos
    //vertices->push_back(1.0f); //yPos - always 0 for now. Going to calculate this on GPU - can change to calclaue it here.
    vertices->push_back(pn);
    vertices->push_back((float)y); //zPos

    // Добавляем текстурные координаты uv в вектор вершин
    vertices->push_back((float)x / (width * stepSize));
    vertices->push_back((float)y / (height * stepSize));
}

// Метод cycleOctaves использует шум Перлина для создания октавного шума с заданным количеством октав.
double Terrain::cycleOctaves(glm::vec3 pos, int numOctaves)
{
    float total = 0.0f;
    float maxAmp = 0.0f;

    float amp = 100.0f;
    float frequency = 0.005f;

    for (int i = 0; i < numOctaves; i++)
    {
        double x = pos.x * frequency;
        double y = pos.y * frequency;
        total += perlin.noise(x, y, 0.01) * amp;
        maxAmp += amp;
        frequency *= 2;
        amp /= 2;
    }
    return (total / maxAmp);
}